# GNU/Linux toolchain for aarch64-unknown-linux-gnu

Reproducibly builds an x86_64 Linux-hosted GCC 13.3.0/binutils 2.42 toolchain
and glibc 2.17 sysroot for `aarch64-unknown-linux-gnu` with crosstool-NG 1.27.0. The producer
...
```sh
gh workflow run forge-conan.yml --repo zackees/forge   -f recipe_repo=zackees/soldr-toolchain -f recipe_ref=<branch>   -f recipe_path=recipes/gnu-linux-toolchain-linux-arm64-gnu   -f name=gnu-linux-toolchain-linux-arm64-gnu   -f version=gcc-13.3.0-glibc-2.17-1 -f linux_x64=true
```

Catalogue path: `gnu-linux-toolchain/gcc-13.3.0-glibc-2.17-1/linux-arm64-gnu/bundle.tar.zst`.
