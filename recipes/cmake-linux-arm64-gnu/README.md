# `cmake-linux-arm64-gnu`

Forge-built Conan recipe — `linux-arm64-gnu` CMake bundle repackaged from
the official upstream prebuilt `cmake-4.3.4-linux-aarch64.tar.gz`
...
```bash
gh workflow run forge-conan.yml --repo zackees/forge     -f recipe_repo=zackees/soldr-toolchain     -f recipe_ref=main     -f recipe_path=recipes/cmake-linux-arm64-gnu     -f name=cmake-linux-arm64-gnu     -f version=4.3.4     -f linux_x64=true     -f windows_x64=false -f macos_arm64=false
```

## Catalogue path after ingest

```
cmake/4.3.4/linux-arm64-gnu/bundle.tar.zst
```
