...
Forge-built Conan recipe — `darwin-arm64` uv bundle repackaged from
the official upstream prebuilt `uv-aarch64-apple-darwin.tar.gz`
...
```bash
gh workflow run forge-conan.yml --repo zackees/forge     -f recipe_repo=zackees/soldr-toolchain     -f recipe_ref=main     -f recipe_path=recipes/uv-darwin-arm64     -f name=uv-darwin-arm64     -f version=0.11.26     -f linux_x64=true     -f windows_x64=false -f macos_arm64=false
```

## Catalogue path after ingest

```
uv/0.11.26/darwin-arm64/bundle.tar.zst
```
