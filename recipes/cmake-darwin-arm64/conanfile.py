"""Conan recipe — `darwin-arm64` CMake prebuilt bundle.
...
Both darwin shapes repackage the same universal
`cmake-<ver>-macos-universal.tar.gz` archive; the
`CMake.app/Contents/` prefix is stripped so the package root is
`bin/` + `share/` like every other shape.
"""
...
class CmakeDarwinArm64(ConanFile):
    name = "cmake-darwin-arm64"
    description = (
        "Pre-built CMake bundle for darwin-arm64. Source: official Kitware/CMake release binaries. Carries bin/cmake + bin/ctest + bin/cpack and the full share/cmake-<maj.min>/ module tree; docs/man dropped."
    )
