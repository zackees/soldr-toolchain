"""Conan recipe — Node.js node.lib import library for Windows arm64.

soldr#944. Sibling of `nodelib-windows-x64/`. Same source archive
(`node-v<ver>-headers.tar.gz` from nodejs.org/dist), different per-arch
import library (`win-arm64/node.lib`).
...
class NodelibWindowsArm64(ConanFile):
    name = "nodelib-windows-arm64"
    description = (
        "Node.js headers + node.lib import library for Windows MSVC aarch64. "
