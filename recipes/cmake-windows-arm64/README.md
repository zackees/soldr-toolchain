...
Forge-built Conan recipe — `windows-arm64` CMake bundle repackaged from
the official upstream prebuilt `cmake-4.3.4-windows-arm64.zip`
...
```bash
gh workflow run forge-conan.yml --repo zackees/forge     -f recipe_repo=zackees/soldr-toolchain     -f recipe_ref=main     -f recipe_path=recipes/cmake-windows-arm64     -f name=cmake-windows-arm64     -f version=4.3.4     -f linux_x64=true     -f windows_x64=false -f macos_arm64=false
```

## Catalogue path after ingest

```
cmake/4.3.4/windows-arm64/bundle.tar.zst
```
