...
def test_latest_is_rejected(tmp_path):
    ...
    path.write_text(json.dumps({"schema_version": 1, "platforms": [str(i) for i in range(8)], "tools": {"cargo-binstall": {"version": "latest", "source": "x", "binary": "x"}, "cargo-nextest": {"version": "1", "source": "x", "binary": "x"}}}))
