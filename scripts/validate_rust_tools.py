...
def validate(path: Path) -> dict:
    ...
    if not isinstance(platforms, list) or len(platforms) != 8 or len(set(platforms)) != 8:
    ...
    for name in ("cargo-binstall", "cargo-nextest"):
        ...
        if not item.get("source") or not item.get("binary"):
            raise ValueError(f"{name} is missing source/binary")
    return doc

def main() -> int:
    parser = argparse.ArgumentParser()
    parser.add_argument("path", type=Path, default=Path(__file__).parents[1] / "managed-rust-tools.json", nargs="?")
